...
function eat() {
  ...
  } else if (bread - Math.round(nutritionalNeeds / 10) < 0){
    nutritionalNeeds -= bread * 10;
    globalVariableData.resources.food.bread = 0;
  };
  //calculates amount of grain to eat after bread is eaten.
  if (nutritionalNeeds > 0 && nutritionalNeeds < grain){
    globalVariableData.resources.food.grain -= nutritionalNeeds;
    nutritionalNeeds = 0;
  }else if (nutritionalNeeds > 0 && nutritionalNeeds > grain){
    nutritionalNeeds -= grain;
    globalVariableData.resources.food.grain = 0;
  };
  //what happens if there wasn't enough food.
  if (nutritionalNeeds > 0){
    globalVariableData.health.hunger -= Math.abs((globalVariableData.population.total - nutritionalNeeds) / globalVariableData.population.total * 10);
  } else if (nutritionalNeeds = 0){
    globalVariableData.health.hunger += 5;
  };
};
